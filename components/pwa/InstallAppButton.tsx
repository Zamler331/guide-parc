...
export default function InstallAppButton() {
  ...
  return (
    <div className="relative z-20 w-full shrink-0 rounded-2xl border border-white/25 bg-white/15 p-[clamp(0.5rem,1.6dvh,0.75rem)] text-left shadow-lg backdrop-blur">
      {installPrompt ? (
        <button
          type="button"
          onClick={handleInstall}
          className="flex min-h-[clamp(2.5rem,6.2dvh,3rem)] w-full items-center justify-center rounded-xl bg-white px-4 text-[clamp(0.78rem,1.85dvh,0.875rem)] font-black leading-tight text-blue-700 shadow-sm transition active:scale-[0.98]"
        >
          Installer l'application
        </button>
      ) : showIosHelp ? (
        <div className="text-[clamp(0.78rem,1.85dvh,0.875rem)] font-bold leading-tight text-white">
